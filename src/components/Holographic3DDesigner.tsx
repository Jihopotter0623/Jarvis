  ...
  useEffect(() => {
    ...
    const render2D = () => {
      ...
      parts.forEach(part => {
        ...
        // Selected HUD overlay text labels in workspace
        if (isSelected) {
          ctx.font = "bold 8px monospace";
          ctx.fillStyle = "#ffffff";
          ctx.globalAlpha = 0.85;
          ctx.fillText(`▶ ${part.name.toUpperCase()}`, pcx + 10, pcy - 4);
          ctx.font = "7px monospace";
          ctx.fillStyle = strokeColor;
          ctx.fillText(`POS: [X:${part.position[0].toFixed(1)} Y:${part.position[1].toFixed(1)} Z:${part.position[2].toFixed(1)}]`, pcx + 10, pcy + 5);
          ctx.fillText(`ROT: [${(part.rotation[0] || 0)}° ${(part.rotation[1] || 0)}° ${(part.rotation[2] || 0)}°]`, pcx + 10, pcy + 13);
        }
      });
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 flex-1">
        ...
        <div className="xl:col-span-6 col-span-12 flex flex-col gap-4">
          <div className="relative rounded-2xl border border-cyan-500/20 bg-slate-950 overflow-hidden flex flex-col group min-h-[480px]">
            ...
            {/* J.A.R.V.I.S. Interactive Voice Assistant HUD Floating Card */}
            <div className="absolute top-16 left-3 z-10 pointer-events-auto flex items-center gap-2.5 bg-slate-950/85 border border-cyan-500/20 px-3 py-2 rounded-xl backdrop-blur-md shadow-[0_0_15px_rgba(6,182,212,0.15)]">
              <div className="relative flex items-center justify-center">
                <span className={`w-3.5 h-3.5 rounded-full ${isListening ? "bg-rose-500 animate-ping" : isSpeaking ? "bg-cyan-400 animate-pulse shadow-[0_0_8px_rgba(34,211,238,0.7)]" : "bg-cyan-600"} border border-slate-950`} />
                <span className={`absolute w-1.5 h-1.5 rounded-full ${isListening ? "bg-rose-400" : isSpeaking ? "bg-cyan-200" : "bg-cyan-400"}`} />
              </div>
              <div className="font-mono text-[9px] text-left">
                <div className="text-cyan-200 font-bold flex items-center gap-1.5">
                  ...
                  {isListening && <span className="text-[7.5px] bg-rose-500/20 text-rose-300 border border-rose-500/30 px-1 py-0.2 rounded animate-pulse">LISTENING</span>}
                  {isSpeaking && <span className="text-[7.5px] bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 px-1 py-0.2 rounded animate-pulse">SPEAKING</span>}
                </div>
                <div className="text-[8px] text-slate-400 mt-0.5 max-w-[150px] truncate">
                  {voiceSubtext}
                </div>
              </div>
              {/* Dynamic bouncing visualizer bars */}
              {(isListening || isSpeaking) && (
                <div className="flex items-end gap-[1.5px] h-3.5 ml-1.5 shrink-0">
                  <span className="w-[1.5px] h-3 bg-cyan-400 rounded-full origin-bottom" style={{ animation: "wave-bounce-1 0.8s infinite ease-in-out" }} />
                  <span className="w-[1.5px] h-4 bg-cyan-300 rounded-full origin-bottom" style={{ animation: "wave-bounce-2 0.6s infinite ease-in-out" }} />
                  <span className="w-[1.5px] h-2.5 bg-cyan-400 rounded-full origin-bottom" style={{ animation: "wave-bounce-3 0.9s infinite ease-in-out" }} />
                  <span className="w-[1.5px] h-3.5 bg-cyan-300 rounded-full origin-bottom" style={{ animation: "wave-bounce-4 0.7s infinite ease-in-out" }} />
                </div>
              )}
            </div>
            
            {/* Viewport Floating Status Overlays */}
            <div className="absolute top-3 left-3 z-10 pointer-events-none text-[8.5px] font-mono text-cyan-400/70 space-y-1 text-left bg-slate-950/80 p-2.5 rounded-xl border border-cyan-500/10 backdrop-blur-md">
              <div className="text-cyan-200 font-bold uppercase tracking-wider">▲ VIEWPORT SHADING GRAPHICS</div>
              <div className="h-[1px] bg-cyan-500/10 my-1" />
              <div>GRID ALIGN: XYZ PLANE FLOORED</div>
              <div>RENDER DEPTH: COMPOSITE (CANVAS_2D)</div>
              <div>ACTIVE NODE: <span className="text-cyan-100 font-bold">{selectedPartId || "None"}</span></div>
              <div className="uppercase">ACTIVE PRESET: {activePreset}</div>
            </div>
            ...
            <div className="absolute bottom-3 left-3 z-10 font-mono text-[9px] bg-slate-950/80 p-2 rounded-xl border border-cyan-500/15 backdrop-blur-md flex items-center gap-3.5">
              ...
              <div className="text-[8px] text-cyan-400 animate-pulse flex items-center gap-1 font-black">
                <Zap className="w-3 h-3" />
                <span>DRAG TO ORBIT // WHEEL TO ZOOM</span>
              </div>
            </div>

            {/* Viewport Statistics overlay in bottom right */}
            <div className="absolute bottom-3 right-3 z-10 pointer-events-none text-[8.5px] font-mono text-slate-500 text-right bg-slate-950/75 p-2 rounded-lg border border-slate-900">
              <div>Polys: {activePreset === "custom" ? customParts.length * 480 : 3824}</div>
              <div>Verts: {activePreset === "custom" ? customParts.length * 240 : 1912}</div>
              <div>System: 60 FPS // CUDA LOCK</div>
            </div>
